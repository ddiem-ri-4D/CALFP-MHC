...
from calfp.BoTNet import LinCALFP, BinCALFP
...
def fix_padding_keys(d):
    ...
    # Renaming keys for BindingDataset compatibility
    if 'peptide_len' in d:
        d['max_len_pep'] = d.pop('peptide_len')
    if 'mhc_len' in d:
        d['max_len_mhc'] = d.pop('mhc_len')
    d.pop('peptide_pad', None)  # Not used in BindingDataset
    return d
...
@click.option('--mode', type=click.Choice(('train', 'eval', '5cv', 'lomo', 'binding', 'seq2logo')), default='train')
...
@click.option('--save-csv', type=click.Path(), default=None, help='Path to save binding predictions as CSV')
