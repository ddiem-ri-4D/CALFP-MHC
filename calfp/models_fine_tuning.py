...
class ModelFineTuning(object):
    """
    Fine-tuning wrapper for CALFP models.
    Handles training, validation, testing, checkpointing.
    """
    ...
    def __init__(self, network, model_path, **kwargs):
        ...
        # Load pretrain weights if exists
        pretrain_model = (
            self.model_path.parent.parent
            .joinpath('pre_models')
            .joinpath(self.model_path.stem + '-epoch-best.pt')
        )
    ...
    # Training & Eval Steps
    # ----------------------
    def train_step(self, inputs: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor],
                   targets: torch.Tensor, **kwargs):
    ...
    def train(self, train_loader: DataLoader, valid_loader: DataLoader, test_loader: DataLoader = None,
              ...
              opt_params: Optional[Mapping] = (), num_epochs=20, **kwargs):
        ...
        for epoch_idx in range(num_epochs):
            ...
            logger.info(f'Epoch: {epoch_idx} '
                        f'-- Loss: {train_loss:.5f} '
                        f'-- Valid: AUC: {auc_valid:.5f} PCC: {pcc_valid:.3f} '
                        f'-- Test: All AUC: {auc_all:.5f} - Group AUC: {auc_group:.5f} - '
                        f'PCC: {pcc_group:.3f} SRCC: {srcc_group:.3f}')

            # Save checkpoint every 5 epochs
            if epoch_idx % 5 == 0:
                save_file = self.model_path.with_name(
                    f'{self.model_path.stem}-epoch-{epoch_idx}{self.model_path.suffix}'
                )
