...
class ModelPretrain(object):
    ...
    def get_scores(self, inputs, **kwargs):
        ...
        return self.model(peptide_x.cuda(), mhc_x.cuda(), peptide_fp.cuda(), mhc_fp.cuda(), **kwargs)
    ...
    def predict(self, data_loader: DataLoader, valid=False, **kwargs):
        ...
        for peptide_x, mhc_x, peptide_fp, mhc_fp, targets in data_loader:
            inputs = (peptide_x, mhc_x, peptide_fp, mhc_fp)
            features.append(self.predict_step(inputs, **kwargs))
    ...
    def save_model(self):
        save_file = self.model_path.with_name(f'{self.model_path.stem}-epoch-best{self.model_path.suffix}')
